...
function Client(options) {
  ...
  this.shardsTotal = options.shardsTotal || 1;
  this.shardNumber = options.shardNumber || 1;
  this.whitelist = options.whitelist ? readWhitelistSync(options.whitelist) : false;
...
var shardIgnored = 0;
var shardUsed = 0;
Client.prototype.useThisShard = function(s3Path, ee){
  var shardToUse = (farmhash.fingerprint32(s3Path) % this.shardsTotal) + 1;
  var use = shardToUse == this.shardNumber;
  if(use){
    shardUsed++
  } else {
    shardIgnored ++
  }
  if((shardIgnored + shardUsed) % 100 == 0){
    ee.shardStats = "used:" + shardUsed + " ignored:" + shardIgnored
  }
  return use
};

function readWhitelistSync(whitelistFile) {
  var whitelist = {};
  var lineByLine = require('n-readlines');
  var liner = new lineByLine(whitelistFile);
...
var whitelistIgnored = 0;
var whitelistUsed = 0;
Client.prototype.isKeyInWhitelist = function(s3Path, ee){
  var use = this.whitelist && !!this.whitelist[s3Path];
  if(use){
    whitelistUsed++
  } else {
    whitelistIgnored ++
  }
  if((whitelistUsed + whitelistIgnored) % 100 == 0){
    ee.whitelistStats = "used:" + whitelistUsed + " ignored:" + whitelistIgnored
  }
  return use
};

Client.prototype.useThisKey = function(s3Path, ee){
  // We may ignore a key because of sharding and/or because we are using a text file to filter which files we deal with
  return this.useThisShard(s3Path,ee) && (this.whitelist ? this.isKeyInWhitelist(s3Path, ee) : true)
};
...
Client.prototype.downloadFile = function(params) {
  ...
  function doTheDownload(cb) {
    inflight++
    console.log("requesting download", inflight, s3Params);
    ...
    request.on('httpHeaders', function(statusCode, headers, resp) {
      ...
      if (headers['content-length'] == undefined) {
        ...
        request.on('httpDone', function() {
          ...
          inflight--;
          console.log("completed download", inflight, s3Params);
          downloader.progressAmount += 1;
          downloader.emit('progress');
          outStream.end();
          cb();
...
Client.prototype.listObjects = function(params) {
  ...
  function findAllS3Objects(marker, prefix, cb) {
    ...
    doWithRetry(listObjects, self.s3RetryCount, self.s3RetryDelay, function(err, data) {
      ...
      var nextMarker = data.NextMarker || data.Contents[data.Contents.length - 1] ? data.Contents[data.Contents.length - 1].Key : null;
      data.Contents = data.Contents.filter(function(entry){
        return self.useThisKey(entry.Key.replace(prefix,''), ee)
      });
      ee.progressAmount += 1;
...
function syncDir(self, params, directionIsToS3) {
  ...
  function findAllFiles(cb) {
    ...
    walker.on('file', function(file, stat, linkPath) {
      ...
      if(self.useThisKey(stat.s3Path, ee)){
        ee.filesFound += 1;
